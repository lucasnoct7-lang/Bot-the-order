...
TOURNAMENT_BALANCE_TIERS: tuple[tuple[str, int, tuple[str, ...]], ...] = (
    ("Especial Grade", 8, ("especial grade", "special grade")),
    ("Grade 1", 7, ("grade 1",)),
    ("Grade 2", 6, ("grade 2",)),
    ("Grade 3", 5, ("grade 3",)),
    ("Grade 4", 4, ("grade 4",)),
    ("Semi Grade 1", 3, ("semi grade 1",)),
    ("Semi Grade 2", 2, ("semi grade 2",)),
    ("Semi Grade 3", 1, ("semi grade 3",)),
    ("Semi Grade 4", 0, ("semi grade 4",)),
)
...
LEGACY_COMMAND_NAMES = (
    ...
    "resetar_tempo_teste_grade",
    "configurar_torneio_pontos",
    "pontuar_participacao",
    "elegibilidade_torneio",
    "resetar_pontos_torneio",
    "saldo",
    "perfil_apostolo",
    "diario",
    "pagar",
...
class ClanCog(commands.Cog):
    ...
    def resolve_tournament_members(
        self,
        guild: discord.Guild,
        raw_value: str,
    ...
    def build_balanced_teams(
        ...
        teams: list[dict[str, Any]] = [
            {"number": team_number, "balance_score": 0, "participants": []}
            for team_number in range(1, team_count + 1)
        ]
        shuffled_participants = participants[:]
        random.shuffle(shuffled_participants)
        shuffled_participants.sort(key=lambda participant: participant.tier_weight, reverse=True)

        for participant in shuffled_participants:
            available_teams = [team for team in teams if len(team["participants"]) < team_size]
            lowest_score = min(team["balance_score"] for team in available_teams)
            lowest_score_teams = [team for team in available_teams if team["balance_score"] == lowest_score]
            selected_team = random.choice(lowest_score_teams)
            selected_team["participants"].append(
                {
                    "user_id": participant.user_id,
                    "user_tag": participant.user_tag,
                    "display_name": participant.display_name,
                    "tier_label": participant.tier_label,
                    "tier_weight": participant.tier_weight,
                    "mention": participant.mention,
                }
            )
            selected_team["balance_score"] += participant.tier_weight
        return teams
    ...
    def build_tournament_embed(
        ...
        mode_label = {"solo": "Solo", "duo": "Duo", "time": "Time"}.get(tournament["mode"], tournament["mode"])
        embed = self.build_embed(tournament["title"], color=discord.Color.dark_gold())
        embed.description = (
            f"**Modo:** {mode_label}\n"
            f"**Jogadores por equipe:** {tournament['team_size']}\n"
            f"**Status:** {tournament['status'].title()}\n"
            f"**ID:** `{tournament['id']}`\n\n"
            "Os times foram sorteados equilibrando os cargos de grade. "
            "Especial Grade recebe maior peso e Semi Grade 4 menor peso."
        )
        for team in teams[:25]:
            roster = "\n".join(
                f"- {participant.get('mention', participant['display_name'])} · {participant['tier_label']}"
                for participant in team["participants"]
            )
            embed.add_field(
                name=f"Equipe {team['number']} · força {team['balance_score']}",
                value=roster or "Sem jogadores",
                inline=False,
            )
        return embed
    ...
    async def criar_torneio(
        ...
        if self.bot.database.get_active_tournament(guild.id) is not None:
            await interaction.response.send_message(
                "Já existe um torneio aberto. Encerre-o antes de criar outro.",
                ephemeral=True,
            )
            return
        ...
        if modo == "solo":
            ...
            team_size = jogadores_por_equipe or 0
            if team_size < 2:
                await interaction.response.send_message(
                    "No modo Time, informe pelo menos 2 jogadores por equipe.",
                    ephemeral=True,
                )
                return
        ...
        if unresolved_entries:
            ...
            await interaction.response.send_message(
                f"Não encontrei estes membros: {unresolved_text}.",
                ephemeral=True,
            )
            return
        if len(resolved_participants) < team_size * 2:
            await interaction.response.send_message(
                "O torneio precisa de pelo menos duas equipes completas.",
                ephemeral=True,
            )
            return
        if len(resolved_participants) > 32:
            await interaction.response.send_message(
                "Use no máximo 32 participantes por torneio.",
                ephemeral=True,
            )
            return
        if len(resolved_participants) % team_size != 0:
            await interaction.response.send_message(
                f"{len(resolved_participants)} participantes não formam equipes completas de {team_size}.",
                ephemeral=True,
            )
            return

        teams = self.build_balanced_teams(resolved_participants, team_size)
        tournament_id = self.bot.database.create_tournament(
            guild_id=guild.id,
            title=(titulo or f"Torneio {modo.title()}").strip()[:100],
            mode=modo,
            team_size=team_size,
        ...
        if tournament is None:
            await interaction.response.send_message("Não consegui salvar o torneio.", ephemeral=True)
            return
        await interaction.response.send_message(embed=self.build_tournament_embed(tournament, teams))

    @app_commands.command(name="ver_torneio", description="Mostra o torneio aberto e suas equipes.")
    async def ver_torneio(self, interaction: discord.Interaction) -> None:
        guild = interaction.guild
        if guild is None:
            await interaction.response.send_message("Esse comando só funciona dentro de um servidor.", ephemeral=True)
            return
        tournament = self.bot.database.get_active_tournament(guild.id)
        if tournament is None:
            await interaction.response.send_message("Não há torneio aberto neste servidor.", ephemeral=True)
            return
        teams = self.bot.database.list_tournament_teams(tournament["id"])
        await interaction.response.send_message(embed=self.build_tournament_embed(tournament, teams))

    @app_commands.command(name="encerrar_torneio", description="Encerra o torneio aberto atual.")
    @app_commands.checks.has_permissions(manage_guild=True)
    async def encerrar_torneio(self, interaction: discord.Interaction) -> None:
        guild = interaction.guild
        if guild is None:
            await interaction.response.send_message("Esse comando só funciona dentro de um servidor.", ephemeral=True)
            return
        tournament = self.bot.database.get_active_tournament(guild.id)
        if tournament is None:
            await interaction.response.send_message("Não há torneio aberto para encerrar.", ephemeral=True)
            return
        if not self.bot.database.close_tournament(guild.id, tournament["id"]):
            await interaction.response.send_message("Não consegui encerrar o torneio.", ephemeral=True)
            return
        await interaction.response.send_message(f"Torneio **{tournament['title']}** encerrado.")
